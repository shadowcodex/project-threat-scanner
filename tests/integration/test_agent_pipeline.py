...
class TestAnalystsPipeline:
    ...
    def test_all_use_bash_in_allowed_tools(self, mock_run):
        ...
        mock_run.return_value = _make_proc(valid_output)
    ...
    @patch("thresher.agents.analysts.subprocess.run")
    def test_all_analysts_get_api_key_in_env(self, mock_run):
        valid_output = json.dumps({
            "analyst": "test",
            "analyst_number": 1,
            "core_question": "test?",
...
class TestAdversarialPipeline:
    ...
    def test_runs_with_high_risk_findings(self, mock_run):
        mock_run.return_value = _make_proc(_load_agent_fixture("adversarial.json"))
    ...
    @patch("thresher.agents.adversarial.subprocess.run")
    def test_skips_no_high_risk(self, mock_run):
        result = run_adversarial_verification(
            _make_config(),
    ...
    @patch("thresher.agents.adversarial.subprocess.run")
    def test_adversarial_max_turns_from_config(self, mock_run):
        mock_run.return_value = _make_proc(_load_agent_fixture("adversarial.json"))
    ...
    @patch("thresher.agents.adversarial.subprocess.run")
    def test_adversarial_default_20_turns(self, mock_run):
        mock_run.return_value = _make_proc(_load_agent_fixture("adversarial.json"))
    ...
    @patch("thresher.agents.adversarial.subprocess.run")
    def test_findings_annotated_with_source_analyst(self, mock_run):
    ...
    @patch("thresher.agents.adversarial.subprocess.run")
    def test_merges_multiple_analyst_findings(self, mock_run):
